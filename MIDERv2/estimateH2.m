...
his = zeros(b,b); %it is probability analogue
x1  = zeros(b,1);
x2  = zeros(b,1);

% Sort values of X and Y (descending order) and preserve elements indexes
[sx,ix] = sort(x,2,'descend');   
[sy,iy] = sort(y,2,'descend');  

% Replace elements of X and Y by their rank order
linsp  = linspace(1,leng,leng);
% assign an order in vector by preserved indexes(now, we have every element is ranged from 0 to leng)
sx(ix) = linsp;
sy(iy) = linsp;
