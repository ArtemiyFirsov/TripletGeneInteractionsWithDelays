...
ntotal     = size(x,2);         %number of elements in the system 
q          = options.q;         %entropic parameter. Used for entropy generalization . if q=1 - entropy of Bolzmann-Gibbs entropy.
fraction   = options.fraction;  %fraction of bins with at least 5 points.
taumax     = options.taumax;    %maximum time lag considered
ert_crit   = options.ert_crit;  %number of entropy reduction rounds.
threshold  = options.threshold; %minimum entropy reduction for a connection
MItype     = options.MItype;	%type of MI used to calculate distance map 
